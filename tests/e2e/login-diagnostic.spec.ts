...
test.describe('Login Page - WITH DIAGNOSTICS', () => {
  test.beforeEach(async ({ page }) => {
    ...
    page.on('response', response => {
      ...
      if (response.url().includes('appwrite')) {
        console.log(`📡 Appwrite response: ${response.status()} ${response.url()}`)
      }
    })
  ...
  test('should successfully login - DIAGNOSTIC VERSION', async ({ page, context }) => {
    ...
    console.log('   Matches expected:', passwordValue === testPassword ? '✅' : '❌')
    ...
    if (cookies.length > 0) {
      ...
      const appwriteCookie = cookies.find(c => 
    ...
    console.log('\n📝 Step 10: Final assertion...')
    await expect(page).toHaveURL('/backlog', { timeout: 30000 })
